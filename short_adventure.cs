	...
	static void Main(string[] args)
		{
			...
			while (play_again)
			{
				...
				{
					Console.WriteLine("I'm sorry"+" "+your_name+" , I don't speak gibberling. Can you plainly say yes or no?: ");
					accept_quest_input = Console.ReadLine().ToLower().Trim();
				}
				if (accept_quest_input == "yes")
				{
					Console.WriteLine(string.Format("Fantastic! Here is the medicine, it is only good for a few days, so please hurry. Take the southern pass and make sure to stay to the left at all costs, good luck, {0}.\nPress any key to leave", your_name));
					Console.ReadKey();
					Console.Clear();
					Console.WriteLine("You leave the guild hall and proceed to head south. You know the way to the nearby village the guild attendant spoke about. It's just over a days travel away.\nAfter an hour you check your gear.\nSword? Check!\nFood? Uh,only one days worth of food.\nYou curse yourself for spending all that money on booze the night before. You have a job to do and this should be enough to get you there.\nPress any key to continue.");
					Console.ReadKey();
					Console.WriteLine("Day 1 Afternoon");
					Console.WriteLine("You continue south towards the mountain until you get to the southern pass. The land is barren, lifeless. The pass goes through a crevice in thhe mountain about two men wide if they were to stand shoulder to shoulder.");
					Console.WriteLine("As you start to head into the crevice, you here a growl behind you. You turn around reflexively to see a grey wolf baring teeth ");
					Console.WriteLine("The wolf takes a step back. You take this time to further inspect the beast. It looks thin, most likely from trying to survive in this meager enviroment. You could probably scare it off with out causing it harm. You could also feed it your only days worth of rations. You won't starve but it will make the trip less enjoyable.");
					Console.WriteLine("Do you feed the wolf or attack? Please type 'attack' or 'feed'");
					string wolf_answer = Console.ReadLine().ToLower().Trim();
				
					if (wolf_answer == "feed")
					{
						Console.Clear();
						Console.WriteLine("You take out your rations amd offer it to the wolf. The wolf eyes your warily, then quickly takes the morsel hungrily.");
						Console.WriteLine("You move towards the crevice, confident that the wolf is content with his meal.");
						Console.WriteLine("Proceeding down the path, you glance back and see that the wolf is following you.\n\"what do you want\" you ask");
						Console.WriteLine("The wolf makes a happy grunting chuffing sound. \n\"Well if you want to come that is fine, but what should I call you?\"");
						Console.WriteLine("What do you call the wolf?: ");
						string wolf_name = Console.ReadLine();
						Console.WriteLine(string.Format("\"{0}, thats a good name, come on lets go.\"\nPress any key to continue.", wolf_name));
						Console.ReadKey();
						...
						{
							Console.Clear();
							Console.WriteLine(string.Format(" You and {0} continue down the path. You come to a intersection, a path going west and a path going east",wolf_name));
							Console.WriteLine("You try to remember what the guild attendent had told you. west or east?");
							string path_choice =  Console.ReadLine().ToLower().Trim();
							while (path_choice != "east" && path_choice != "west")
							{
								Console.WriteLine("The only direction to move forward is to go is east or west.");
								path_choice =  Console.ReadLine().ToLower().Trim();
							}
							if (path_choice == "east"){break;}
							if (i == 0) {dead = true; break;}
							if (i == 1){Console.WriteLine("You decide to go along the west path again. This time taking your time navigating the slopes. more hours pass but you reach the top. You over look the pass and it seems like the only way forward is back down the slope you fell before. You navigate it carefully to be back in front of the original path.\nPress any key to continue"); Console.ReadKey();}
							else 
							{
							Console.WriteLine(" You choose the west path. It is  rocky and treacherous. Hours have passed as you finally reach the top of the path you stumble, sliding down a slope. Battered and bruised you pick yourself up. You recognize that you are back on the original path before the intersection.\nPress any key to continue.");
							Console.ReadKey();
							}
